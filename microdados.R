...
# Este código irá criar um gráfico de barras mostrando o número de mortes por município, com os municípios ordenados 
# pelo número de mortes. Isso proporciona uma visualização clara da distribuição de óbitos por município.
# Relacionando as tabelas pelo código do município

# Agrupando os dados por município e contando o número de mortes em cada um
mortes_por_municipio <- microdadosobitos2019 %>%
  group_by(codmunres) %>%
  summarise(numero_de_mortes = n()) %>%
  top_n(10, numero_de_mortes)  # Selecionando os 10 maiores municípios

# Adicionando os nomes dos municípios
mortes_por_municipio <- inner_join(mortes_por_municipio, tabmunpais, by = c("codmunres" = "codmun"))

# Calculando as porcentagens de mortes por município
mortes_por_municipio <- mortes_por_municipio %>%
  mutate(percentual = numero_de_mortes / sum(numero_de_mortes) * 100)

# Excluindo São Paulo dos 10 principais municípios
mortes_por_municipio <- mortes_por_municipio %>%
  filter(nome_municipio != "São Paulo")

# Reordenando os níveis do fator nome_municipio de acordo com o número de mortes
